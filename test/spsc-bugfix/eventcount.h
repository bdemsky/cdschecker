...
unsigned get ( ) {
unsigned cmp = count . fetch_or ( 0x80000000 , std :: memory_order_seq_cst ) ;
...
private : std :: atomic < unsigned > count ;
...
if ( cmp & 0x80000000 ) {
guard . lock ( $ ) ;
while ( false == count . compare_exchange_weak ( cmp , ( cmp + 1 ) & 0x7FFFFFFF , std :: memory_order_relaxed ) ) ;
unsigned w = waiters ( $ ) ;
waiters = 0 ;
guard . unlock ( $ ) ;
if ( w ) cv . notify_all ( $ ) ;
