...
;
struct mcs_mutex {
public : std :: atomic < mcs_node * > m_tail ;
mcs_mutex ( ) {
__sequential_init();
m_tail . store ( NULL ) ;
}
...
class guard {
public : mcs_mutex * m_t ;
mcs_node m_node ;
guard ( mcs_mutex * t ) : m_t ( t ) {
t -> lock ( this ) ;
}
~ guard ( ) {
m_t -> unlock ( this ) ;
...
inline static call_id_t Unlock_id() {
call_id_t __ID__ = 0;
return __ID__;
}
...
inline static bool Unlock_check_action(void *info, call_id_t __ID__) {
bool check_passed;
...
inline static call_id_t Lock_id() {
call_id_t __ID__ = 0;
return __ID__;
}
...
inline static bool Lock_check_action(void *info, call_id_t __ID__) {
bool check_passed;
...
bool success ;
success = m_tail . compare_exchange_strong ( tail_was_me , NULL , std :: mo_acq_rel ) ;
...
rl :: linear_backoff bo ;
for ( ; ; ) {
next = me -> next . load ( std :: mo_acquire ) ; if ( next != NULL ) break ; thrd_yield ( ) ; }
}
